...
<div class="form-card" style="max-width:780px;">
    <h5 class="mb-4" style="color:var(--azul-java);font-weight:700;">
        <i class="fa-solid fa-pen me-2" style="color:var(--vermelho-express);"></i>Editar Pedido #{{ $pedido->id }}
    </h5>
    ...
    <form method="POST" action="{{ route('admin.pedidos.update', $pedido) }}" id="pedido-form">
        @csrf @method('PUT')

        <div class="row g-3 mb-3">
            <div class="col-md-6">
                <label class="form-label">Nome do Cliente <span class="text-danger">*</span></label>
                <input type="text" name="cliente_nome" class="form-control" value="{{ old('cliente_nome', $pedido->cliente_nome) }}" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">E-mail do Cliente</label>
                <input type="email" name="cliente_email" class="form-control" value="{{ old('cliente_email', $pedido->cliente_email) }}">
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-select" required>
                @foreach($statusLabels as $key => $label)
                    <option value="{{ $key }}" {{ old('status', $pedido->status) == $key ? 'selected' : '' }}>{{ $label }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <div id="itens-container">
                @foreach($pedido->produtos as $index => $prod)
                <div class="item-linha row g-2 mb-2 align-items-center">
                    <div class="col-7">
                        <select name="produtos[]" class="form-select produto-select" required>
                            ...
                            @foreach($produtos as $p)
                                <option value="{{ $p->id }}" data-preco="{{ $p->preco }}" {{ $prod->id == $p->id ? 'selected' : '' }}>
                    ...
                    <div class="col-3">
                        <input type="number" name="quantidades[]" class="form-control qtd-input" value="{{ $prod->pivot->quantidade }}" min="1">
                    </div>
                    <div class="col-2">
        ...
        <div class="mb-4">
            ...
            <textarea name="observacoes" class="form-control" rows="2">{{ old('observacoes', $pedido->observacoes) }}</textarea>
        </div>

        <div class="d-flex gap-2">
            <button type="submit" class="btn btn-express px-4">
                <i class="fa-solid fa-floppy-disk me-2"></i>Atualizar Pedido
            </button>
